...
Models and summaries are saved in the logs/tf directory. They are identified by a unique key generated by the model parameters. For example, here's a model directory with 
...
* num_epochs = 10
    ...
    directory: logs/tf/runs/Em-128_FS-3-4-5_NF-128_D-0.5_L2-0.01_B-64_Ep-10/

To activate tensorboard during model training, run the following from the project root directory:
...
The script `mood_classification.py` is available to use as a full pipeline from labeled lyrics to embeddings to CNN. To use it, just run `python mood_classification.py`. Note that there are *many* controls available within the main() function.

### Predictions

The predictions notebook is available to review CNN results. Just point it to a dumped confusion_matrix.csv to visualize the matrix and measure the accuracy.
