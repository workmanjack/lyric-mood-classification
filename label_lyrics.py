"""
Use this script to append mood labels to the index csv generated by index_lyrics.py.

This script uses the Last.fm sqlite database to match moods to tracks. Depending on the arguments used,
the mood categories will either be from the original or the expanded set.

For more information on how to use the Last.fm sqlite database, please see this useful tutorial from MSD:
https://labrosa.ee.columbia.edu/millionsong/sites/default/files/lastfm/demo_tags_db.py

Recommended Command:

    python label_lyrics.py --expanded-moods
    
Output: a data/labeled_lyrics.csv with track information and mood label.
"""
import os
import sys
import time
